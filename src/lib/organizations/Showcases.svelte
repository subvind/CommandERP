<script lang="ts">
  ...
  let url: string = `https://backend.subvind.com/showcases/orgRelated/${organization.id}`;
  let limit: number = 25;
  let columns = [
    {
      id: 'id',
    ...
    { 
      ...
      formatter: (cell: any, row: any) => {
        return gridjs.h('a', {
          href: `/${organization.owner.username}/${organization.orgname}/showcases/${row.cells[1].data}`,
          target: "_self",
